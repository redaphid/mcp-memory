...
async function test() {
  ...
  try {
    const storeResponse = await fetch(`${BASE_URL}/user/test/sse`, {
      method: 'POST',
  ...
  try {
    ...
    const searchResponse = await fetch(`${BASE_URL}/user/test/sse`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        jsonrpc: '2.0',
  ...
  try {
    const searchAll = await fetch(`${BASE_URL}/user/test/sse`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        jsonrpc: '2.0',
    ...
    const allData = await searchAll.json()
